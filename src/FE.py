...
def add_country(df):
    ...
    df["ORIGIN_COUNTRY"] = df_us + df_can
...
def encoding(df):
    month_cyclical = CyclicalFeatures(variables=None, drop_original=True)
    pu_month = month_cyclical.fit_transform(df["PU_MONTH"].to_frame())
    dl_month = month_cyclical.fit_transform(df["DL_MONTH"].to_frame())
    df = df.join(pu_month)
    df = df.join(dl_month)
    df = df.drop(columns=["PU_MONTH", "DL_MONTH"])

    # Save encoder
    dump(month_cyclical, "month_cyclical.joblib")

    weekday_cyclical = CyclicalFeatures(variables=None, drop_original=True)
    pu_weekday = weekday_cyclical.fit_transform(df["PU_WEEKDAY"].to_frame())
    dl_weekday = weekday_cyclical.fit_transform(df["DL_WEEKDAY"].to_frame())
    df = df.join(pu_weekday)
    df = df.join(dl_weekday)
    df = df.drop(columns=["PU_WEEKDAY", "DL_WEEKDAY"])

    # Save encoder
    dump(weekday_cyclical, "weekday_cyclical.joblib")

    hour_cyclical = CyclicalFeatures(variables=None, drop_original=True)
    pu_hour = hour_cyclical.fit_transform(df["PU_HOUR"].to_frame())
    dl_hour = hour_cyclical.fit_transform(df["DL_HOUR"].to_frame())
    df = df.join(pu_hour)
    df = df.join(dl_hour)
    df = df.drop(columns=["PU_HOUR", "DL_HOUR"])

    # Save encoder
    dump(hour_cyclical, "hour_cyclical.joblib")

    minute_cyclical = CyclicalFeatures(variables=None, drop_original=True)
    pu_minute = minute_cyclical.fit_transform(df["PU_MINUTE"].to_frame())
    dl_minute = minute_cyclical.fit_transform(df["DL_MINUTE"].to_frame())
    df = df.join(pu_minute)
    df = df.join(dl_minute)
    df = df.drop(columns=["PU_MINUTE", "DL_MINUTE"])

    # Save encoder
    dump(minute_cyclical, "minute_cyclical.joblib")
